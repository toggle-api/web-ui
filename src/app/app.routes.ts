...
export const components: any[] = routes.filter(route => route.children !== undefined).map(route => route.children).concat(routes).filter((route: Route) => route.component !== undefined).map((route: Route) => route.component);
